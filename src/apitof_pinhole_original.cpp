...
// Evaluate velocity and angular velocity in reference system of collision
void collision(double * v_cluster, double * omega, double u_norm, double v_cluster_norm, double costheta, double sintheta, float R_cluster, double vib_energy, double vib_energy_old, float m_ion, float m_gas, float temperature, ofstream  &warnings, int &nwarnings)
{
  double phi;
  double alpha;
  ...
  // Define reference system aligning ion velocity with z1-axis
  //norm_v_cluster=sqrt(v_cluster[0]*v_cluster[0]+v_cluster[1]*v_cluster[1]+v_cluster[2]*v_cluster[2]);
  if(v_cluster_norm>0)
  {
    for(int i=0;i<3;i++)
    {
  ...
  // Evaluate versor of collision = y3
  for(int i=0; i<3;i++)
  {
    y3[i]=sintheta*cosphi*x1[i]+sintheta*sinphi*y1[i]+costheta*z1[i];
  }

  // Define reference system aligning z2-axis perpendicular to ion velocity
  cross_norm(z1,y3,z2);
  cross_norm(y3,z2,x2);
  // Define reference system aligning the tangential component of carrier gas molecule velocity with x3-axis
  alpha=2.0*pi*unif(gen);
  cosalpha=cos(alpha);
  sinalpha=sin(alpha);
  for(int i=0;i<3;i++)
  {
    x3[i]=cosalpha*x2[i]+sinalpha*z2[i];
    z3[i]=-sinalpha*x2[i]+cosalpha*z2[i];
  }
  //cout << vec_norm(x3) << endl;
  //cout << vec_norm(y3) << endl;
  //cout << vec_norm(z3) << endl;
  ...
  u[0]=onedimMaxwell(m_gas,kT);
  //u[0]=0.0; // TO REMOVE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  u[1]=-u_norm;

  // Evaluate new velocities after collision
